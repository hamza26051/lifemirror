...
def _exp(minutes: int = 30) -> int:
    return int((datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(minutes=minutes)).timestamp())

def _exp_days(days: int) -> int:
    return int((datetime.datetime.now(datetime.timezone.utc) + datetime.timedelta(days=days)).timestamp())

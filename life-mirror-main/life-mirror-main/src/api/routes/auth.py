...
async def login(data: LoginIn, db: Session = Depends(get_db)):
    ...
    # Only use Redis-based rate limiting if Redis is available
    if r:
        try:
    ...
    if not user or not user.password_hash or not verify_password(data.password, user.password_hash):
        # Only track failures if Redis is available
        if r:
            try:
                ...
                pass  # Continue without Redis tracking
        raise HTTPException(status_code=status.HTTP_401_UNAUTHORIZED, detail="Invalid credentials")

    # success → reset counters if Redis is available
    if r:
        try:
            await r.delete(key)
            await r.delete(lock_key)
        except Exception:
            pass  # Continue without Redis cleanup

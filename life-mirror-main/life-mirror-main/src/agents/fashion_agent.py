...
class FashionAgent(BaseAgent):
    ...
    def run(self, input: AgentInput) -> AgentOutput:
        ...
        for d in detections:
            ...
            os.makedirs("uploads/fashion", exist_ok=True)
            filename = f"{input.media_id}_{len(items)}.jpg"
            local_path = os.path.join("uploads", "fashion", filename)
            
            try:
                cv2.imwrite(local_path, crop)
                crop_url = f"http://localhost:8000/storage/fashion/{filename}"
            except Exception:
                # fallback: keep original image url if save fails
                crop_url = input.url
        ...
        if len(items) > 0:
            ...
            confidence = min(0.95, 0.7 + (avg_score * 0.2) + (len(items) * 0.05))
        else:
            confidence = 0.5  # Lower confidence when no items detected
        
        result = AgentOutput(success=True, data={"style": style, "items": items, "overall_rating": overall_rating, "confidence": confidence})
        self._trace(input.dict(), result.dict())
        return result
